...
EntryArray.prototype.add = EntryArray.prototype.addEntry;
...
EntryArray.prototype.removeEntry = function(timestamp, value) {
    ...
    for (; i < arr.length; i++) {
        ...
        matchesTimestamp = !hasTimestamp || (arr[i] === timestamp || arr[i].ts === timestamp);
        matchesValue = !hasValue || arr[i].val === value;
        if (matchesTimestamp && matchesValue) {
            arr[i] = undefined;
        } else {
            //if we need to move it back, do that now
            if (i !== j) {
...
EntryArray.prototype.remove = EntryArray.prototype.removeEntry;
